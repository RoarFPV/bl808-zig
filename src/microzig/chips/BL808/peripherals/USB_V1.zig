...
pub const Type = extern struct {
    ...
    // 0x20
    @"USB interrupt enable": mmio.Mmio(packed struct(u32) {
        usb_cr_sof_en: u1,
    ...
    // 0x24
    @"USB interrupt status": mmio.Mmio(packed struct(u32) {
        usb_sof_int: u1,
        usb_reset_int: u1,
        usb_vbus_tgl_int: u1,
    ...
    // 0x28
    @"USB interrupt mask": mmio.Mmio(packed struct(u32) {
        usb_cr_sof_mask: u1,
    ...
    // 0x2c
    @"USB interrupt clear": mmio.Mmio(packed struct(u32) {
        usb_cr_sof_clr: u1,
